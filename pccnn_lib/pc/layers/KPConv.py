...
class KPConv(IConvLayer):
    ...
    def __init__(self, 
        ...
        if np_kernel_pts.shape[-1] == 3:
            np_kernel_pts, _ = rotate_pc_3d(np.random.RandomState(15), np_kernel_pts)
        ...
        self.register_buffer('sigma_', torch.tensor((min_distance*1.0)/(1.0+min_distance)))
    ...
    def reset_parameters(self):
        """Reset parameters.
        """
        stdv = math.sqrt(2.0 / (self.feat_input_size_*self.num_kernel_pts_))
    ...
    def __compute_convolution__(self,
        ...
        if self.wamup_state_ and self.use_const_variance_init_:
            ...
            variance = ComputeKPConvWeightVar.apply(
...
class KPConvFactory(IConvLayerFactory):
    ...
    def __init__(self, 
        ...
        # Super class init.
        super(KPConvFactory, self).__init__(
            p_num_kernel_pts,
            p_const_var_w_init,
            p_const_var_value)
