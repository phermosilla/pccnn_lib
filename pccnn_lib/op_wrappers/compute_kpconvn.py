...
class ComputeKPConvN(torch.autograd.Function):
    """Function to compute a Monte Carlo convolution.
    """

    @staticmethod
    def forward(p_ctx, p_pts, p_pdfs, p_features, p_samples, p_neighbors, 
        ...
        p_ctx.save_for_backward(
            p_pts, p_pdfs, p_features, p_samples, p_neighbors, 
            p_start_ids, p_radii, p_kernel_pts, p_conv_weights)
        p_ctx.sigma_ = p_sigma

        # Compute the pdf.
        result_tensor = pccnn_lib_ops.kp_conv_n(
            p_pts, p_pdfs, p_features, p_samples, p_neighbors, 
            p_start_ids, p_radii, p_kernel_pts, p_sigma, p_conv_weights)
    ...
    def backward(p_ctx, p_grads):
        ...
        return pt_grads, None, feat_grads, sample_grads, None, None, \
            None, kernel_pts_grads, None, weight_grads


class ComputeKPConvNWeightVar(torch.autograd.Function):
    ...
    def forward(p_ctx, p_pts, p_pdfs, p_features, p_samples, p_neighbors, 
        ...
        # Compute the pdf.
        result_tensor = pccnn_lib_ops.kp_conv_n_weight_var(
            p_pts, p_pdfs, p_features, p_samples, p_neighbors, 
            p_start_ids, p_radii, p_kernel_pts, p_sigma)
