...
def test_mcconv_pt_grads(p_config_dict, p_data):
    ...
    np_conv_weights = np.random.normal(0.0, 1.0, (
    ...
    torch_pdf = torch.as_tensor((pc.pts_unnorm_pdf_ * mul_val).detach().cpu().numpy(), device=device)
    torch_pdf.requires_grad = True
    ...
    def compute_conv_pts(points):
        pc.pts_ = points
        pc.pts_pdf_ = None
        pc.compute_pdf(badnwidth_np)
        mul_val = torch.prod(radius)
        cur_pdf = pc.pts_unnorm_pdf_ * mul_val
        #pc.pts_pdf_ = torch.ones_like(pc.pts_pdf_)
    ...
    with warnings.catch_warnings():
        ...
        test = torch.autograd.gradcheck(compute_conv_pts, 
            torch_pts, eps=1e-3, atol = 1e-2, rtol = 1e-2, nondet_tol = 1e-2, 
            raise_exception=True)

        test = test and torch.autograd.gradcheck(compute_conv_pdf, 
            torch_pdf, eps=1e-1, atol = 1e-3, rtol = 1e-3, nondet_tol = 1e-3, 
            raise_exception=True)
